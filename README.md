...
- 可在模型配置弹窗修改 Base URL、基础模型名和图像模型名，并一键恢复原始默认值
...
Base URL 与模型名默认填入 `.env.local` 中的原配置，也可在页面修改并保存到当前浏览器 `localStorage`。每次模型调用会携带当前连接参数；服务端校验后转发。API Key 同样由用户在页面填写；正式生产建议改用 Secret Manager。
